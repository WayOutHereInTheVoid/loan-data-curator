...
let hasMore = true;
  ...
  const reviewed = allData.filter(item => item.review_status && item.review_status !== 'pending').length;
  const research_further = allData.filter(item => item.review_status === 'research_further').length;
  const doesnt_work = allData.filter(item => item.review_status === 'doesnt_work').length;
  const use = allData.filter(item => item.review_status === 'use').length;

...
import { ChevronLeft, ChevronRight, Heart, Trash2, MessageSquare, RotateCcw, BarChart3 } from 'lucide-react';
...
export default function LoanDataCurator() {
  ...
  const loadData = async () => {
    setLoading(true);
    let query = supabase.from('DataPts').select('*');
  ...
  const loadStats = async () => {
    ...
    const reviewed = data.filter(item => item.status !== 'pending').length;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading data points...</p>
        </div>
      </div>
    );
  }

  const progressPercentage = dataPoints.length > 0 ? ((currentIndex / dataPoints.length) * 100) : 0;
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      ...
      {currentDataPoint ? (
        <div className="max-w-md mx-auto">
          <div
            ref={cardRef}
            className="bg-white rounded-xl shadow-xl p-6 border border-gray-200 cursor-grab active:cursor-grabbing transform transition-transform duration-200"
            style={{
          ...
          <div className="grid grid-cols-4 gap-3 mt-6">
            <button
              ...
              className="p-3 rounded-xl border-2 text-red-600 bg-red-50 border-red-200 hover:bg-red-100 transition-colors"
            >
              <Trash2 size={24} className="mx-auto" />
            </button>
            <button
              onClick={() => handleAction('notes')}
              className="p-3 rounded-xl border-2 text-blue-600 bg-blue-50 border-blue-200 hover:bg-blue-100 transition-colors"
            >
              <MessageSquare size={24} className="mx-auto" />
            </button>
            <button
              onClick={() => handleAction('favorite')}
              className="p-3 rounded-xl border-2 text-purple-600 bg-purple-50 border-purple-200 hover:bg-purple-100 transition-colors"
            >
              <Heart size={24} className="mx-auto" />
            </button>
            <button
              onClick={() => handleAction('keep')}
              className="p-3 rounded-xl border-2 text-green-600 bg-green-50 border-green-200 hover:bg-green-100 transition-colors"
            >
        ...
        <div className="max-w-md mx-auto text-center">
          <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-200">
            ...
            <p className="text-gray-600 mb-6">
              You&apos;ve reviewed all {dataPoints.length} data points in this category.
            </p>
